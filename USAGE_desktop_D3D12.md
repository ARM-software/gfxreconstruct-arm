...
Refresh Opened Shared Resource Contents | GFXRECON_CAPTURE_OPEN_SHARED_RESOURCE_REFRESH | BOOL | Direct3D 12 only. Refresh the contents of externally produced resources opened with `ID3D12Device::OpenSharedHandle` before each Present. Default is: `true`. Set to `false` to retain only the snapshot before the first shader-resource consumer submission, reducing capture size at the cost of not replaying later external-producer updates.
